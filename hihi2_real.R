...
theta_true = seq(0, 0.5, by=(1/2)*(1/(graph_size-1)))

#theta_true = seq(0, 1, by=1/(graph_size-1))
#theta_true = sort(runif(n=(graph_size), min=(1/(graph_size-1)), max=1 ))


theta_hw = function(th) c(th^2,2*th*(1-th),(1-th)^2)
theta_cube = function(th) c((th)^4,1*(th)^2*(1/2-(th))^2,(1-(th))^4)
theta_cube = function(th) c((th/2)^3,(th/2)^2,(th/2))
...
X_hw = matrix(0,nrow=n,ncol=3)
for(i in 1:n) X_hw[i,] = theta_hw(theta_true[i])
...
d=W.Xhat[,1]
b1_hw = 2*sum(t^2*d)
b2_hw  = 4*sum(t*d) - 4*sum(t^2*d)  
b3_hw  =  2*sum(d)- 4*sum(t*d)+ 2*sum(t^2*d) 
