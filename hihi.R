...
####X_sorted = X_noise[order( X_noise[,1] ),]
...
# plot(theta_sorted, theta_noise_fit_sorted)
 ...
 X_naive_sorted_sim = theta_sorted+rnorm(n=length(theta_sorted), mean=0, sd=5*sqrt(sigma_u_theta_true_sorted))
...
#Z = cbind(y, X_naive_big)
#Sigma_U_theta=sigma_u_theta_true
...
M_z = (1/graph_size)*M
#beta_adj[[r]] = solve(M_z[2,2])%*%M_z[2,1]
...
for (i in 1:nrow(X)){
  ...
  result <- optim(par = 0.5, min.RSS_hw, data = dat, method = "L-BFGS-B", lower = 0, upper = 1.0)
  theta_noise_hw[i] <- result$par
}

#CHEAT:
flip = matrix(c(0, 0, 1, 0, 1, 0, 1, 0, 0), byrow=T, nrow=3)
X_naive_hw = theta_noise_hw
if(sum((theta_noise_hw-theta_true)^2)>sum(((1-theta_noise_hw)-theta_true)^2)){
  X_naive_hw = 1-theta_noise_hw
  theta_noise_hw = 1-theta_noise_hw
  #print("1")
  Xr_noise_simp = (Xr_noise_simp) %*% flip
}

X_naive_big_hw = cbind(rep(1, graph_size), X_naive_hw)
Z_hw = cbind(y, X_naive_big_hw)
Sigma_U_theta_hw=sigma_u_theta_true

M_hw = matrix(0,nrow=3,ncol=3)
for (i in (1:graph_size)){
  M_hw = (Z_hw)[i,]%o%Z_hw[i,]-diag(c(0, 0, Sigma_U_theta_hw[i])) + M_hw
}
M_z_hw = (1/graph_size)*M_hw
#beta_adj[[r]] = solve(M_z[2,2])%*%M_z[2,1]
beta_adj_true_hw[[r]] = solve(M_z_hw[2:3,2:3])%*%M_z_hw[2:3,1]

beta_naive_hw[[r]] = solve(t(X_naive_big_hw)%*%(X_naive_big_hw))%*%t(X_naive_big_hw)%*% y
...
beta_adj_true_hw_se[[r]] = (beta_adj_true_hw[[r]]-c(1, 1))^2
beta_naive_hw_se[[r]] = (beta_naive_hw[[r]]-c(1, 1))^2
...
b0_adj_naive_se_sim = c()
b1_adj_naive_se_sim = c()
b0_adj_est_se_sim = c()
b1_adj_est_se_sim = c()
b0_naive_hw_sim = c()
b1_naive_hw_sim = c()
b0_adj_true_hw_sim = c()
b1_adj_true_hw_sim = c()
b0_naive_hw_se_sim = c()
b1_naive_hw_se_sim = c()
b0_adj_true_hw_se_sim = c()
b1_adj_true_hw_se_sim = c()
...
for (z in 1:(r-1)){
  ...
  b0_adj_naive_se_sim[z] = beta_adj_naive_se[[z]][1]
  b1_adj_naive_se_sim[z] = beta_adj_naive_se[[z]][2]
  
  b0_adj_naive_sim[z] = beta_adj_naive[[z]][1]
  b1_adj_naive_sim[z] = beta_adj_naive[[z]][2]
  ...
  b0_naive_hw_sim[z] = beta_naive_hw[[z]][1]
  b1_naive_hw_sim[z] = beta_naive_hw[[z]][2]
  b0_adj_true_hw_sim[z] = beta_adj_true_hw[[z]][1]
  b1_adj_true_hw_sim[z] = beta_adj_true_hw[[z]][2]
  
  b0_naive_hw_se_sim[z] = beta_naive_hw_se[[z]][1]
  b1_naive_hw_se_sim[z] = beta_naive_hw_se[[z]][2]
  b0_adj_true_hw_se_sim[z] = beta_adj_true_hw_se[[z]][1]
  b1_adj_true_hw_se_sim[z] = beta_adj_true_hw_se[[z]][2]
