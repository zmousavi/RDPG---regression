...
#Simulate RDPG graph to generate X_hat.
...
#Generate RDPG
A_temp = apply(P_true,c(1,2),function(g) rbinom(1, 1, g)) 
A=matrix(0, nrow=graph_size, ncol=graph_size)
A[upper.tri(A)] = A_temp[upper.tri(A_temp)]
A =  A+t(A)
diag(A) = diag(A_temp)
A.data = A
svd_A = eigen(A)
...
#Construct X_hat 
X_hat=svd_A$vectors[,1:3]%*%diag(sqrt(svd_A$values[1:3]))
Xr_noise = X_hat 
...
if(sum((Xr_noise[end1_index,]-X[1,])^2)>(sum((Xr_noise[end1_index,]-X[3,])^2))){
  temp=end1_index
  end2_index = end1_index
  end1_index=temp
}

foo = g_dist_m[end1_index,]-g_dist_m[end2_index,]
mid_index = which(foo==min(abs(foo)))
if (is.integer(mid_index) && length(mid_index)==0) {mid_index = which(foo==-1*min(abs(foo)))[[1]]} 

Xr_noise_pts = rbind(Xr_noise[end1_index,], Xr_noise[mid_index,], Xr_noise[end2_index,])
...
Xr_noise_hw = t(rbind(theta_noise_hw^2, 2*theta_noise_hw*(1-theta_noise_hw), (1-theta_noise_hw)^2))

#Compute an estimate for the covariance of error
